     ...
     <div className='bg-white border border-gray-300 rounded text-sm max-h-[80vh] min-h-[60vh] overflow-y-scroll'>
       ...
       </div>
       {
         appointments.reverse().map(( item, index )=>(
           <div className='flex flex-wrap justify-between max-sm:gap-2 sm:grid sm:grid-cols-[0.5fr_3fr_3fr_3fr_1fr_1fr] items-center text-gray-500 py-3 px-6 border border-gray-200 hover:bg-gray-50' key={index}>
             ...
             <div className='flex items-center gap-2'>
               <img className='w-8 rounded-full bg-gray-200' src={ item.serviceData.image } alt="user image" /> <p>{ item.serviceData.name }</p>
             </div>
             <p>{ currency }{ item.amount }</p>
             {
             item.cancelled
             ? <p onClick={()=>cancelAppointment( item._id )} className='text-red-400 text-xs font-medium cursor-pointer'>Cancelled</p>
             :
             item.isCompleted ?
