...
const UpdateAppointments = () => {
 ...
 const { aToken, updateAppointments, completeAppointment, cancelAppointment } = useContext(ServiceContext)
         ...
         appointments.reverse().map(( item, index )=>(
           <div className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1fr] gap-1 items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-50' key={ index }>
             ...
             <div className='flex items-center gap-2'>
               ...
               <div>
                 <p className='text-xs inline border border-primary-500 px-2 rounded-full'>
               ...
               <p>{ currency }{ item.amount }</p>
               {
                 ...
                 <div className='flex'>
                 <img onClick={()=>cancelAppointment( item._id )} className='w-10 cursor-pointer' src={ assets.cancel_icon } alt="users" />
                 <img onClick={()=>completeAppointment( item._id )} className='w-10 cursor-pointer' src={ assets.tick_icon } alt="woman" />
               </div>
               }
